...
class PantheonMaterialStockService
{
    ...
    public function canIssue(ConnectionInterface $db, string $warehouse, array $items): bool
    {
        ...
        foreach ($items as $item) {
            ...
            $required[$key] = [
                'code' => $code,
                'quantity' => bcadd(WorkOrderClosingCalculator::decimal($required[$key]['quantity'] ?? '0'), WorkOrderClosingCalculator::decimal($item['quantity']), WorkOrderClosingCalculator::SCALE),
            ];
        }
        ...
        foreach ($required as $item) {
            ...
            if ($row === null || bccomp(WorkOrderClosingCalculator::decimal($row->anStock), $item['quantity'], WorkOrderClosingCalculator::SCALE) < 0) {
                return false;
            }
        }
    ...
    private function change(ConnectionInterface $db, string $warehouse, string $ident, string $delta, Carbon $now, int $userId, bool $allowInsert): void
    {
        ...
        $next = bcadd(WorkOrderClosingCalculator::decimal($row->anStock), WorkOrderClosingCalculator::decimal($delta), WorkOrderClosingCalculator::SCALE);
